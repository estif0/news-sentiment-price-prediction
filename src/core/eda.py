...
class EDAAnalyzer:
    ...
    def extract_common_keywords(
        self, column: str = "headline", top_n: int = 10
    ) -> List[Tuple[str, int]]:
        """
    ...
    def parse_dates(self, date_column: str = "date") -> pd.Series:
        ...
        if self._parsed_dates is None:
            ...
            self._parsed_dates = pd.to_datetime(
                self.df[date_column], errors="coerce", utc=True
            )

        return self._parsed_dates

    def analyze_publication_frequency(
        self, date_column: str = "date", freq: str = "D"
    ) -> pd.Series:
        """
    ...
    def analyze_publishing_times(self, date_column: str = "date") -> pd.Series:
        ...
        if self._publishing_times is None:
            ...
            self._publishing_times = (
                self.df[date_column].dt.hour.value_counts().sort_index()
            )
        return self._publishing_times

    def extract_publisher_domains(
        self, publisher_column: str = "publisher"
    ) -> pd.Series:
        """
        ...
        if self._publisher_domains is None:
            if publisher_column not in self.df.columns:
                raise ValueError(
                    f"DataFrame must contain a '{publisher_column}' column."
                )

            self._publisher_domains = (
                self.df[publisher_column]
                .astype(str)
                .apply(lambda x: x.split("@")[-1] if "@" in x else None)
            )
        return self._publisher_domains

    def analyze_publisher_stock_counts(
        self, publisher_column: str = "publisher", stock_column: str = "stock"
    ) -> pd.DataFrame:
        """
        ...
        if self._publisher_stock_counts is None:
            if (
                publisher_column not in self.df.columns
                or stock_column not in self.df.columns
            ):
                raise ValueError(
                    f"DataFrame must contain '{publisher_column}' and '{stock_column}' columns."
                )

            self._publisher_stock_counts = (
                self.df.groupby([publisher_column, stock_column])
                .size()
